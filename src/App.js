...
export default function App() {
  ...
  return (
    <div style={{ minHeight: "100vh", display: "flex", background: "var(--paper)" }}>
      <aside style={{
        width: sidebar ? 248 : 0, minWidth: sidebar ? 248 : 0,
        ...
        <div style={{ padding: "28px 20px 20px", borderBottom: "1px solid rgba(255,255,255,0.08)" }}>
          <div style={{ fontFamily: "'Instrument Serif', serif", fontStyle: "italic", fontSize: "1.6rem", marginBottom: 16 }}>MyNotes</div>
        ...
        <div style={{ padding: "14px 20px", borderBottom: "1px solid rgba(255,255,255,0.08)", display: "flex", gap: 16 }}>
          {[{ label: "Notes", val: notes.length }, { label: "Pinned", val: pinnedCount }, { label: "JSON", val: jsonCount }].map(s => (
          ...
          {[null, ...TAGS_LIST].map(t => {
            ...
            return (
              <button key={t || "all"} onClick={() => { setFilterTag(t); setSidebar(false); }} style={{
                ...
                fontFamily: "'Syne', sans-serif", fontWeight: filterTag === t ? 800 : 600,
                fontSize: "0.85rem", cursor: "pointer", transition: "all 0.15s", marginBottom: 2,
              }}>
        ...
        <div style={{ padding: "16px 20px", borderTop: "1px solid rgba(255,255,255,0.08)" }}>
          <button onClick={() => signOut(auth)} style={{
            ...
            borderRadius: 10, padding: "9px", color: "#fca5a5",
            fontFamily: "'Syne', sans-serif", fontWeight: 800, fontSize: "0.82rem", cursor: "pointer",
          }}>🚪 Sign Out</button>
        </div>
      </aside>
        ...
        <header style={{
          ...
          {filterTag && (
            <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
              ...
              <span style={{ background: "var(--ink)", color: "white", borderRadius: 20, padding: "3px 12px", fontSize: "0.76rem", fontWeight: 800, display: "inline-flex", alignItems: "center", gap: 6 }}>
                #{filterTag}
                <button onClick={() => setFilterTag(null)} style={{ background: "none", border: "none", cursor: "pointer", color: "white", fontSize: 15, lineHeight: 1, padding: 0 }}>×</button>
              </span>
            </div>
          )}
        </header>
        ...
        <main style={{ flex: 1, padding: "18px 16px", maxWidth: 900, margin: "0 auto", width: "100%" }}>
          {loading ? (
            ...
            <div style={{ columnCount: 2, columnGap: 14 }}>
              {filtered.map(note => <NoteCard key={note.id} note={note} onEdit={openEdit} />)}
            </div>
          )}
        </main>
      </div>
      ...
      <button onClick={openNew} style={{
        ...
        color: "white", border: "none", borderRadius: "50%", fontSize: 28, cursor: "pointer",
        boxShadow: "0 6px 28px rgba(26,26,46,0.35)", zIndex: 500,
        display: "flex", alignItems: "center", justifyContent: "center",
      }}>＋</button>

      {editorOpen && <NoteEditor note={editNote} user={user} onClose={() => { setEditorOpen(false); setEditNote(null); }} />}
    </div>
  );
    }
